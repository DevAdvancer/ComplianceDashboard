...
import { createExcelBuffer } from "@/lib/exports/report";
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return new NextResponse(createExcelBuffer(record), {
    headers: {
      "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      "Content-Disposition": `attachment; filename="${record.candidate_name.replace(/\s+/g, "-").toLowerCase()}-report.xlsx"`,
    },
  });
}
